...
export class RandomForest {
    ...
    predictClass(sample: { a: number, b: number }) {
        return this.predict(sample, 'class')
    }

    predictProbability(sample: { a: number, b: number }) {
        return this.predict(sample, 'probability')
    }

    predict(sample: { a: number, b: number }, type: 'class' | 'probability') {
        type = type || 'class'
        const results = []
    ...
    evaluate(samples: { a: number, b: number }[]) {
        ...
        each(report.class, d => {
            ...
            (<number>d['fscore']) = 2 / (1 / d['precision'] + 1 / d['recall'])
...
export class DecisionTree {
    ...
    predict(sample: { a: number, b: number }) {
        let root = this.model
...
function createTree(_s: { a: number, b: number, output: number }[], target: string, features: string[]) {
...
function gain(_s: { a: number, b: number, output: number }[], target: string, feature: string) {
